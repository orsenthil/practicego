...
func main() {
	// TODO: Create an FX application using fx.New()
	// Provide the constructors: NewGreeter, NewPrinter
	// Invoke a function that takes *Printer and calls p.Print()
	// Finally, call Run() on the app
	
	// Example structure:
	// fx.New(
	//     fx.Provide(NewGreeter, NewPrinter),
	//     fx.Invoke(func(p *Printer) { p.Print() }),
	// ).Run()
}
